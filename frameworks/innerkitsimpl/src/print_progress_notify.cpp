...
PrintProgressNotify::PrintProgressNotify(
    napi_env env, const std::string &type, napi_ref ref)
...
/*
    ...
    PrintNapiUtils::SetUint32Property(env, resultPrintMargin, "bottom", info.GetCapability().GetMinMargin().GetBottom());
    PrintNapiUtils::SetUint32Property(env, resultPrintMargin, "left", info.GetCapability().GetMinMargin().GetLeft());
    PrintNapiUtils::SetUint32Property(env, resultPrintMargin, "right", info.GetCapability().GetMinMargin().GetRight());
    uint32_t printerCapabilityLength = info.GetCapability().GetPageSize().size();

    for(uint32_t i = 0; i < printerCapabilityLength; i++)
    {
        napi_value resultPrinterPageSize;
        napi_create_object(env, &resultPrinterPageSize);
        PrintNapiUtils::SetUint32Property(env, resultPrinterPageSize, "id", info.GetCapability().GetPageSize()[i].GetId());
        PrintNapiUtils::SetStringPropertyUtf8(env, resultPrinterPageSize, "name", info.GetCapability().GetPageSize()[i].GetName().c_str());
        PrintNapiUtils::SetUint32Property(env, resultPrinterPageSize, "width", info.GetCapability().GetPageSize()[i].GetWidth());
        PrintNapiUtils::SetUint32Property(env, resultPrinterPageSize, "height", info.GetCapability().GetPageSize()[i].GetHeight());
    ...
    for(uint32_t i = 0; i < printerCapabilityresolutionLength; i++)
    {
        ...
        PrintNapiUtils::SetUint32Property(env, resultPrinterResolution, "id", info.GetCapability().GetResolution()[i].GetId());
        PrintNapiUtils::SetUint32Property(env, resultPrinterResolution, "horizontalDpi", info.GetCapability().GetResolution()[i].GetHorizontalDpi());
        PrintNapiUtils::SetUint32Property(env, resultPrinterResolution, "verticalDpi", info.GetCapability().GetResolution()[i].GetVerticalDpi());
    ...
    work->data = notifyData;
        ...
        [](uv_work_t *work, int statusInt) {
            ...
            if (notifyData != nullptr) {
                ...
                if("PrinterInfo" == notifyData->objectType)
                {
                    WriteInfoJsObject(notifyData->env, notifyData->secondArgv, &callbackVal[PrintNapiUtils::SECOND_ARGV]);
                }
                else if("PrintJob" == notifyData->objectType)
                {
                    WriteJobJsObject(notifyData->env, notifyData->thirdArgv, &callbackVal[PrintNapiUtils::SECOND_ARGV]);
                }
                napi_call_function(notifyData->env, nullptr, callbackFunc, PrintNapiUtils::TWO_ARG, callbackVal, &result);
                if (work != nullptr) {
                    delete work;
                    work = nullptr;
                }
