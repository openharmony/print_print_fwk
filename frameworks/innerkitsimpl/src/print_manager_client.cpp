...
bool PrintManagerClient::On(const std::string &type, uint32_t &state,
                            PrinterInfo &info,
...
bool PrintManagerClient::RegisterExtCallback(uint32_t callbackId,
                                             PrintExtCallback cb) {
  if (printServiceProxy_ == nullptr) {
    PRINT_HILOGW("Redo GetPrintServiceProxy");
    printServiceProxy_ = GetPrintServiceProxy();
  ...
  PRINT_HILOGD("PrintManagerClient RegisterExtCallback %{public}s.",
               ret ? "success" : "failed");
  return ret;
}
...
bool PrintManagerClient::UnregisterAllExtCallback() {
  ...
  PRINT_HILOGD("PrintManagerClient UnregisterAllExtCallback %{public}s.",
               ret ? "success" : "fail");
  if (ret) {
...
bool PrintManagerClient::QueryAllExtension(
...
bool PrintManagerClient::StartDiscoverPrinter(
...
bool PrintManagerClient::RemovePrinters(
...
bool PrintManagerClient::UpdatePrinterState(uint32_t printerId,
                                            uint32_t state) {
  if (printServiceProxy_ == nullptr) {
    PRINT_HILOGW("Redo GetPrintServiceProxy");
    printServiceProxy_ = GetPrintServiceProxy();
...
bool PrintManagerClient::UpdatePrinterJobState(uint32_t jobId, uint32_t state) {
  ...
  if (printServiceProxy_ == nullptr) {
    PRINT_HILOGE("UpdatePrinterJobState quit because redoing "
                 "GetPrintServiceProxy failed.");
    return false;
  }
...
bool PrintManagerClient::RequestPreview(const PrintJob &jobinfo,
...
bool PrintManagerClient::QueryPrinterCapability(
  ...
  return printServiceProxy_->QueryPrinterCapability(printerId,
                                                    printerCapability);
}
