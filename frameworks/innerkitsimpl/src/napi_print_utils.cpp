...
napi_value NapiPrintUtils::GetNamedProperty(napi_env env, napi_value object,
  ...
  bool hasProperty = false;
  PRINT_CALL(env, napi_has_named_property(env, object, propertyName.c_str(),
                                          &hasProperty));
  if (!hasProperty) {
    return value;
  }
  PRINT_CALL(
      env, napi_get_named_property(env, object, propertyName.c_str(), &value));
  return value;
}
...
std::string NapiPrintUtils::GetLocalId(const std::string &globalId,
...
std::string NapiPrintUtils::GetTaskEventId(const std::string &taskId,
