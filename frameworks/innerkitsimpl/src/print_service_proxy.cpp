...
int32_t PrintServiceProxy::StartPrint() {
  ...
  PRINT_HILOGD("PrintServiceProxy StartPrint succeeded. TaskId = %{public}d",
               taskId);
  return taskId;
}
...
bool PrintServiceProxy::QueryAllExtension(
...
bool PrintServiceProxy::StartDiscoverPrinter(
    const std::vector<uint32_t> &extensionList) {
  MessageParcel data, reply;
  MessageOption option;
  data.WriteInterfaceToken(PrintServiceProxy::GetDescriptor());
  data.WriteUint32(extensionList.size());
  PRINT_HILOGD("OnStartDiscoverPrinter extensionList.size() = %{public}lu",
               extensionList.size());
...
bool PrintServiceProxy::AddPrinters(
    const std::vector<PrinterInfo> &arrayPrintInfo) {
  MessageParcel data, reply;
  MessageOption option;
  data.WriteInterfaceToken(PrintServiceProxy::GetDescriptor());
  data.WriteUint32(arrayPrintInfo.size());
  PRINT_HILOGD("OnStartDiscoverPrinter arrayPrintInfo.size() = %{public}lu",
               arrayPrintInfo.size());
...
bool PrintServiceProxy::RemovePrinters(
    const std::vector<PrinterInfo> &arrayPrintInfo) {
  MessageParcel data, reply;
  MessageOption option;
  data.WriteInterfaceToken(PrintServiceProxy::GetDescriptor());
...
bool PrintServiceProxy::UpdatePrinterJobState(uint32_t jobId, uint32_t state) {
  ...
  PRINT_HILOGD("PrintServiceProxy ChangeTaskPriority started.");
  bool ret =
      Remote()->SendRequest(CMD_UPDATEPRINTERJOBSTATE, data, reply, option);
  if (ret != ERR_NONE) {
    PRINT_HILOGD("PrintServiceProxy, ret = %{public}d", ret);
    return false;
  }
...
bool PrintServiceProxy::RequestPreview(const PrintJob &jobinfo,
...
bool PrintServiceProxy::QueryPrinterCapability(
    uint32_t printerId, PrinterCapability &printerCapability) {
  MessageParcel data, reply;
  MessageOption option;
  data.WriteInterfaceToken(PrintServiceProxy::GetDescriptor());
  data.WriteUint32(printerId);
...
bool PrintServiceProxy::CheckPermission() {
  ...
  int32_t result =
      Remote()->SendRequest(CMD_CHECKPERMISSION, data, reply, option);
  if (result != ERR_NONE) {
    PRINT_HILOGE(" PrintServiceProxy::CheckPermission fail, ret = %{public}d ",
                 result);
    return false;
  }
...
bool PrintServiceProxy::On(const std::string &type, uint32_t &state,
                           PrinterInfo &info,
...
bool PrintServiceProxy::RegisterExtCallback(
    uint32_t callbackId, const sptr<IPrintExtensionCallback> &listener) {
  ...
  if (result != ERR_NONE) {
    PRINT_HILOGE(
        "PrintServiceProxy::RegisterExtCallback fail, ret = %{public}d",
        result);
    return false;
  }
  bool ret = reply.ReadBool();
...
bool PrintServiceProxy::UnregisterAllExtCallback() {
  ...
  if (result != ERR_NONE) {
    PRINT_HILOGE(
        " PrintServiceProxy::UnregisterAllExtCallback fail, ret = %{public}d ",
        result);
    return false;
  }
  bool ret = reply.ReadBool();
  PRINT_HILOGD(
      "PrintServiceProxy::UnregisterAllExtCallback out [ret: %{public}d]", ret);
  return ret;
}

void PrintServiceProxy::BuildParcelFromPrintJob(MessageParcel &data,
...
void PrintServiceProxy::BuildParcelFromPrinterInfo(
    MessageParcel &data, const PrinterInfo &printerInfo) {
