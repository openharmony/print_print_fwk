...
PrintJob::PrintJob() : jobId_(""), printerId_(""), jobState_(PRINT_JOB_PREPARED), subState_(PRINT_JOB_BLOCKED_UNKNOWN),
    copyNumber_(0), isSequential_(false), isLandscape_(false), colorMode_(0), duplexMode_(0), margin_(nullptr),
    preview_(nullptr), option_("") {
}
...
bool PrintJob::CreateFileList(napi_env env, napi_value &jsPrintJob) const
{
    ...
    for (uint32_t i = 0; i < arrFilesLength; i++) {
        napi_value value;
        PRINT_CALL_BASE(env, napi_create_string_utf8(env, files_[i].c_str(), NAPI_AUTO_LENGTH, &value), false);
        PRINT_CALL_BASE(env, napi_set_element(env, arrFiles, i, value), false);
    }
    PRINT_CALL_BASE(env, napi_set_named_property(env, jsPrintJob, PARAM_JOB_FILES, arrFiles), false);
    return true;
}
