...
void PrintJob::SetJobId(uint32_t jobId) { jobId_ = jobId; }

void PrintJob::SetPrintId(uint32_t printerid) { printerId_ = printerid; }

void PrintJob::SetJobState(uint32_t jobState) { jobState_ = jobState; }
...
uint32_t PrintJob::GetJobId() const { return jobId_; }

uint32_t PrintJob::GetPrinterId() const { return printerId_; }
...
void PrintJob::ConvertToJs(napi_env env, napi_value *result) const {
  ...
  for (uint32_t i = 0; i < arrFilesLength; i++) {
    napi_value value;
    status = napi_create_string_utf8(env, files_[i].c_str(), NAPI_AUTO_LENGTH,
                                     &value);
  ...
  NapiPrintUtils::SetUint32Property(env, *result, "copyNumber",
                                    GetCopyNumber());
  napi_value pageRange;
  napi_create_object(env, &pageRange);
  NapiPrintUtils::SetUint32Property(env, pageRange, "startPage",
                                    pageRange_.GetStartPage());
  NapiPrintUtils::SetUint32Property(env, pageRange, "endPage",
                                    pageRange_.GetEndPage());
  napi_value arrPages;
  ...
  NapiPrintUtils::SetUint32Property(env, *result, "isSequential",
                                    GetIsSequential());
  napi_value pageSize;
  napi_create_object(env, &pageSize);
  NapiPrintUtils::SetStringPropertyUtf8(env, pageSize, "id", pageSize_.GetId());
  NapiPrintUtils::SetStringPropertyUtf8(env, pageSize, "name",
                                        pageSize_.GetName().c_str());
  NapiPrintUtils::SetUint32Property(env, pageSize, "width",
                                    pageSize_.GetWidth());
  NapiPrintUtils::SetUint32Property(env, pageSize, "height",
                                    pageSize_.GetHeight());

  NapiPrintUtils::SetUint32Property(env, *result, "isLandscape",
                                    GetIsLandscape());
  ...
  NapiPrintUtils::SetStringPropertyUtf8(env, preview, "result",
                                        preview_.GetResult().c_str());
  napi_value subPageRange;
  napi_create_object(env, &subPageRange);
  NapiPrintUtils::SetUint32Property(env, subPageRange, "startPage",
                                    pageRange_.GetStartPage());
  NapiPrintUtils::SetUint32Property(env, subPageRange, "endPage",
                                    pageRange_.GetEndPage());
