...
public:
  ...
  virtual bool
  StartDiscoverPrinter(const std::vector<uint32_t> &extensionList) = 0;
  virtual bool StopDiscoverPrinter() = 0;
  virtual bool
  QueryAllExtension(std::vector<PrintExtensionInfo> &arrayExtensionInfo) = 0;
  ...
  virtual bool
  RemovePrinters(const std::vector<PrinterInfo> &arrayPrintInfo) = 0;
  ...
  virtual bool RequestPreview(const PrintJob &jobinfo,
                              std::string &previewResult) = 0;
  virtual bool QueryPrinterCapability(uint32_t printerId,
                                      PrinterCapability &printerCapability) = 0;
  virtual bool CheckPermission() = 0;
  virtual bool On(const std::string &type, uint32_t &state, PrinterInfo &info,
                  const sptr<IPrintCallback> &listener) = 0;
  virtual bool Off(const std::string &type) = 0;
  virtual bool
  RegisterExtCallback(uint32_t callbackId,
