...
void JsPrintExtension::Init(const std::shared_ptr<AbilityLocalRecord> &record,
  ...
  PRINT_HILOGD("JsPrintExtension::Init module:%{public}s,srcPath:%{public}s.",
  ...
  auto shellContextRef = jsRuntime_.LoadSystemModule(
...
void JsPrintExtension::RegisterAllCallback() {
  ...
  PrintManagerClient::GetInstance()->RegisterExtCallback(
      PRINT_EXTCB_START_DISCOVERY, []() -> bool {
  ...
  PrintManagerClient::GetInstance()->RegisterExtCallback(
      PRINT_EXTCB_STOP_DISCOVERY, []() -> bool {
  ...
  PrintManagerClient::GetInstance()->RegisterExtCallback(
      PRINT_EXTCB_CONNECT_PRINTER, [](uint32_t printId) -> bool {
        PRINT_HILOGD("Connect Printer");
        HandleScope handleScope(jsExtension_->jsRuntime_);
        NativeEngine *nativeEng = &(jsExtension_->jsRuntime_).GetNativeEngine();
        napi_value jsPrintId = OHOS::AppExecFwk::WrapInt32ToJS(
            reinterpret_cast<napi_env>(nativeEng),
  ...
  PrintManagerClient::GetInstance()->RegisterExtCallback(
      PRINT_EXTCB_START_PRINT, [](const PrintJob &job) -> bool {
        PRINT_HILOGD("Start Print Job");
        HandleScope handleScope(jsExtension_->jsRuntime_);
        NativeEngine *nativeEng = &(jsExtension_->jsRuntime_).GetNativeEngine();
        napi_value jobObject = NapiPrintUtils::Convert2JsObj(
            reinterpret_cast<napi_env>(nativeEng), job);
  ...
  PrintManagerClient::GetInstance()->RegisterExtCallback(
      PRINT_EXTCB_CANCEL_PRINT, [](const PrintJob &job) -> bool {
  ...
  PrintManagerClient::GetInstance()->RegisterExtCallback(
      ...
      [](uint32_t printId, PrinterCapability &cap) -> bool {
        ...
        if (result != nullptr) {
          ...
          cap.BuildFromJs(reinterpret_cast<napi_env>(nativeEng),
