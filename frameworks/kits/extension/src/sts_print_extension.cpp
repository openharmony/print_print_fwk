...
static constexpr char PRINT_EXTENSION_CLASS_NAME[] = "L@ohos/app/ability/PrintExtensionAbility;";
...
StsPrintExtension* StsPrintExtension::Create(const std::unique_ptr<Runtime>& runtime)
{
    ...
    stsExtension_ = new (std::nothrow) StsPrintExtension(static_cast<STSRuntime&>(*runtime));
    return stsExtension_;
}

StsPrintExtension::StsPrintExtension(STSRuntime &stsRuntime) : stsRuntime_(stsRuntime) {}
...
std::shared_ptr<STSNativeReference> StsPrintExtension::LoadModule(ani_env *env)
{
    std::shared_ptr<STSNativeReference> stsNativeReference = std::make_shared<STSNativeReference>();
    ani_class cls = nullptr;
    ani_status status = ANI_ERROR;
    if ((status = env->FindClass(PRINT_EXTENSION_CLASS_NAME, &cls)) != ANI_OK) {
