...
private:
    ...
    napi_value OnConnectAbilityWithAccount(napi_env &engine, napi_callback_info &info)
    {
        ...
        params.connection->SetJsConnectionObject(argv[1]);
        {
            ...
            ConnecttionKey key;
            key.id = serialNumber_;
            key.want = params.want;
            connects_.emplace(key, params.connection);
            (serialNumber_ < INT64_MAX) ? serialNumber_++ : serialNumber_ = 0;
        }
        NapiAsyncTask::CompleteCallback complete = [weak = context_, params](
                                                       napi_env engine, NapiAsyncTask &task, int32_t status) {
            CompleteConnectAbilityWithAccount(engine, task, weak, params);
        };
        napi_value result = nullptr;
