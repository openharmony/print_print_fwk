...
public:
   ...
   */
  virtual void
  Init(const std::shared_ptr<AppExecFwk::AbilityLocalRecord> &record,
       const std::shared_ptr<AppExecFwk::OHOSApplication> &application,
       std::shared_ptr<AppExecFwk::AbilityHandler> &handler,
       const sptr<IRemoteObject> &token) override;

  /**
   * @brief Called when this extension is started. You must override this
   ...
   * extension.
   * @param Want Indicates the {@link Want} structure containing startup
   ...
   *
   * @param want Indicates the {@link Want} structure containing connection
   * information about the Print extension.
   * @return Returns a pointer to the <b>sid</b> of the connected Print
   * extension.
   */
  virtual sptr<IRemoteObject> OnConnect(const AAFwk::Want &want) override;

  /**
   * @brief Called when all abilities connected to this Print extension are
   * disconnected.
   *
   * You can override this function to implement your own processing logic.
   *
   ...
   * This method can be called only by Print. You can use the
   * StartAbility(ohos.aafwk.content.Want) method to start Print. Then the
   * system calls back the current method to use the transferred want parameter
   ...
   * Print is restarted after being destroyed, and the value false indicates a
   * normal startup.
   * @param startId Indicates the number of times the Print extension has been
   ...
   */
  virtual void OnCommand(const AAFwk::Want &want, bool restart,
...
private:
  NativeValue *CallObjectMethod(const char *name,
                                NativeValue *const *argv = nullptr,
                                size_t argc = 0);
