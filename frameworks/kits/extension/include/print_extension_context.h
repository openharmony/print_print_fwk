...
public:
   ...
   * An ability using the AbilityInfo.AbilityType.Print or
   ...
   * AbilityInfo.AbilityType.Print template.
   *
   ...
   */
  bool
  ConnectAbility(const AAFwk::Want &want,
  ...
  ErrCode
  StartAbilityWithAccount(const AAFwk::Want &want, int accountId,
                          const AAFwk::StartOptions &startOptions) const;
   ...
   */
  ErrCode
  DisconnectAbility(const AAFwk::Want &want,
                    const sptr<AbilityConnectCallback> &connectCallback) const;
...
protected:
  bool IsContext(size_t contextTypeId) override {
    return contextTypeId == CONTEXT_TYPE_ID ||
           ExtensionContext::IsContext(contextTypeId);
  }
