...
{
    smb2_destroy_context_func_t fn;
    {
        std::lock_guard<std::mutex> lock(mutex_);
...
{
    smb2_disconnect_share_func_t fn;
    {
        std::lock_guard<std::mutex> lock(mutex_);
...
{
    smb2_share_enum_async_func_t fn;
    {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    return fn(ctx, static_cast<enum SHARE_INFO_enum>(level), callback, privateData);
}

void SmbLibrary::FreeData(struct smb2_context* ctx, void* data) const
{
    smb2_free_data_func_t fn;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        fn = smb2_free_data_;
    }
...
{
    smb2_set_security_mode_func_t fn;
    {
        std::lock_guard<std::mutex> lock(mutex_);
