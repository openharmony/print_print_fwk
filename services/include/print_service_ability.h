...
public:
  ...
  bool QueryAllExtension(
      std::vector<PrintExtensionInfo> &arrayExtensionInfo) override;
  bool
  StartDiscoverPrinter(const std::vector<uint32_t> &extensionList) override;
  ...
  bool RequestPreview(const PrintJob &jobinfo,
  ...
  static void NotifyPrintStateHandler(const std::string &type, uint32_t state,
                                      PrinterInfo info);
  static void NotifyJobStateHandler(const std::string &type, uint32_t state,
                                    PrintJob job);
