...
bool PrintServiceManager::On(const std::string &type, uint32_t &state,
                             PrinterInfo &info,
...
bool PrintServiceManager::StartDiscoverPrinter(
...
bool PrintServiceManager::QueryExtensionAbilityInfos(
  ...
  arrayExtensionInfo[NapiPrintUtils::ARGC_ZERO].SetExtensionId(
      NapiPrintUtils::MAX_ARGC);
  arrayExtensionInfo[NapiPrintUtils::ARGC_ZERO].SetVendorId(
      NapiPrintUtils::MAX_ARGC);
  ...
  arrayExtensionInfo[NapiPrintUtils::ARGC_ZERO].SetVendorIcon(
      NapiPrintUtils::MAX_ARGC);
...
bool PrintServiceManager::RemovePrinters(
    std::vector<PrinterInfo> arrayPrintInfo) {
  PRINT_HILOGE("PrintServiceManager RemovePrinters started.");
  return true;
}

bool PrintServiceManager::UpdatePrinterState(uint32_t printerId,
                                             uint32_t state) {
  PRINT_HILOGE("PrintServiceManager UpdatePrinterState started.");
  return true;
}

bool PrintServiceManager::UpdatePrinterJobState(uint32_t jobId,
                                                uint32_t state) {
  PRINT_HILOGE("PrintServiceManager UpdatePrinterJobState started.");
  return true;
}

bool PrintServiceManager::RequestPreview(PrintJob jobinfo,
...
bool PrintServiceManager::QueryPrinterCapability(
...
void PrintServiceManager::InstallCallback(uint32_t taskId,
                                          PrintTaskCallback eventCb) {
