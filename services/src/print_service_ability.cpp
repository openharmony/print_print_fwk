...
PrintServiceAbility::PrintServiceAbility(int32_t systemAbilityId,
    ...
    : SystemAbility(systemAbilityId, runOnCreate),
      state_(ServiceRunningState::STATE_NOT_START), currentTaskId_(0) {}
...
int32_t PrintServiceAbility::StartPrint() {
  ...
  PRINT_HILOGI("PrintServiceAbility Allocatejjjjjj Task[%{public}d] started.",
               taskId);
  return taskId;
}
...
bool PrintServiceAbility::StartDiscoverPrinter(
...
bool PrintServiceAbility::QueryAllExtension(
...
bool PrintServiceAbility::AddPrinters(
...
bool PrintServiceAbility::RemovePrinters(
    const std::vector<PrinterInfo> &arrayPrintInfo) {
  ManualStart();
  auto instance = PrintServiceManager::GetInstance();
  if (instance == nullptr) {
...
bool PrintServiceAbility::UpdatePrinterState(uint32_t printerId,
                                             uint32_t state) {
  ManualStart();
  auto instance = PrintServiceManager::GetInstance();
  if (instance == nullptr) {
...
bool PrintServiceAbility::UpdatePrinterJobState(uint32_t jobId,
                                                uint32_t state) {
  ManualStart();
  auto instance = PrintServiceManager::GetInstance();
  if (instance == nullptr) {
...
bool PrintServiceAbility::RequestPreview(const PrintJob &jobinfo,
...
bool PrintServiceAbility::QueryPrinterCapability(
...
bool PrintServiceAbility::RegisterExtCallback(
...
bool PrintServiceAbility::On(const std::string &type, uint32_t &state,
                             PrinterInfo &info,
                             const sptr<IPrintCallback> &listener) {
  std::string combineType = type + "-";
  PRINT_HILOGI("PrintServiceAbility::On started. type=%{public}s",
               combineType.c_str());
  return true;
}
...
bool PrintServiceAbility::Off(const std::string &type) {
  std::string combineType = type + "-";
  PRINT_HILOGI("PrintServiceAbility::Off started.type=%{public}s",
               combineType.c_str());
  return false;
}
...
void PrintServiceAbility::NotifyPrintStateHandler(const std::string &type,
                                                  uint32_t state,
                                                  PrinterInfo info) {
  std::string combineType = type;
  PRINT_HILOGI(
      "PrintServiceAbility::NotifyHandler combineType %{public}s [%{public}d.",
      combineType.c_str(), state);
}

void PrintServiceAbility::DataWriteJob(PrintJob job, MessageParcel &data) {}

void PrintServiceAbility::NotifyJobStateHandler(const std::string &type,
                                                uint32_t state, PrintJob job) {
  std::string combineType = type;
  PRINT_HILOGI(
      "PrintServiceAbility::NotifyHandler combineType %{public}s [%{public}d.",
      combineType.c_str(), state);
