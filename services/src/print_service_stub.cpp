...
PrintServiceStub::PrintServiceStub() {
  ...
  cmdMap_[CMD_UPDATEPRINTERJOBSTATE] =
      &PrintServiceStub::OnUpdatePrinterJobState;
...
bool PrintServiceStub::OnQueryAllExtension(MessageParcel &data,
  ...
  if (result) {
    ...
    for (uint32_t i = 0; i < printerInfoLegth; i++) {
      PRINT_HILOGI("PrintServiceProxy, ExtensionId = %{public}d",
                   printerInfo[i].GetExtensionId());
      PRINT_HILOGI("PrintServiceProxy, VendorId = %{public}d",
                   printerInfo[i].GetVendorId());
      PRINT_HILOGI("PrintServiceProxy, VendorName = %{public}s",
                   printerInfo[i].GetVendorName().c_str());
      PRINT_HILOGI("PrintServiceProxy, VendorIcon = %{public}d",
                   printerInfo[i].GetVendorIcon());
      PRINT_HILOGI("PrintServiceProxy, Version = %{public}s",
                   printerInfo[i].GetVersion().c_str());
...
bool PrintServiceStub::OnAddPrinters(MessageParcel &data,
  ...
  PRINT_HILOGD("OnStartDiscoverPrinter arrayPrintInfoLength = %{public}d",
               arrayPrintInfoLength);
...
bool PrintServiceStub::OnUpdatePrinterJobState(MessageParcel &data,
                                               MessageParcel &reply) {
...
bool PrintServiceStub::OnCheckPermission(MessageParcel &data,
                                         MessageParcel &reply) {
...
bool PrintServiceStub::OnEventOn(MessageParcel &data, MessageParcel &reply) {
  ...
  PRINT_HILOGD(
      "PrintServiceStub::OnEventOn taskId = %{public}d type=%{public}s ",
      taskId, type.c_str());
  if (type.empty()) {
...
bool PrintServiceStub::OnEventOff(MessageParcel &data, MessageParcel &reply) {
  ...
  std::string type = data.ReadString();
  PRINT_HILOGD(
      "PrintServiceStub::OnEventOff taskId = %{public}d type=%{public}s ",
      taskId, type.c_str());
...
void PrintServiceStub::MakePrinterInfo(MessageParcel &data,
                                       PrinterInfo &printerInfo) {
