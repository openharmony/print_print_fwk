...
napi_value NapiPrintExt::AddPrinters(napi_env env, napi_callback_info info) {
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    context->result = PrintManagerClient::GetInstance()->AddPrinters(
        context->printAddInfoVector);
...
napi_value NapiPrintExt::RemovePrinters(napi_env env, napi_callback_info info) {
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    context->result = PrintManagerClient::GetInstance()->RemovePrinters(
        context->printRemoveInfoVector);
    if (context->result == true) {
      context->status = napi_ok;
    }
  };
...
napi_value NapiPrintExt::UpdatePrinterState(napi_env env,
  ...
  auto input = [context](napi_env env, size_t argc, napi_value *argv,
    ...
    NAPI_CALL_BASE(
        ...
        napi_invalid_arg);
    NAPI_ASSERT_BASE(env, valuetype == napi_number, "valuetype is not a number",
                     napi_invalid_arg);
    ...
    NAPI_CALL_BASE(
        env,
        napi_typeof(env, argv[NapiPrintUtils::INDEX_ONE], &valueSecondtype),
        napi_invalid_arg);
    NAPI_ASSERT_BASE(env, valueSecondtype == napi_number,
                     "valueSecondtype is not a string", napi_invalid_arg);
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    context->result = PrintManagerClient::GetInstance()->UpdatePrinterState(
...
napi_value NapiPrintExt::UpdatePrintJobState(napi_env env,
  ...
  auto input = [context](napi_env env, size_t argc, napi_value *argv,
    ...
    NAPI_ASSERT_BASE(env, valuetype == napi_number,
                     "printerJobState is not a number", napi_invalid_arg);
    ...
    napi_get_value_uint32(env, argv[NapiPrintUtils::INDEX_ONE],
                          &printerJobState);
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    context->result = PrintManagerClient::GetInstance()->UpdatePrinterJobState(
        context->printerJobId, context->printerJobState);
    if (context->result == true) {
      context->status = napi_ok;
    }
  };
