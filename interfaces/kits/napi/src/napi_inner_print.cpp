...
napi_value NapiInnerPrint::QueryExtensionInfo(napi_env env,
  ...
  auto output = [context](napi_env env, napi_value *result) -> napi_status {
    ...
    uint32_t PrinterExtensionInfoLength =
        context->arrayPrinterExtensionInfo.size();
    for (uint32_t i = 0; i < PrinterExtensionInfoLength; i++) {
      PRINT_HILOGD("PrintServiceProxy, ExtensionId = %{public}d",
                   context->arrayPrinterExtensionInfo[i].GetExtensionId());
      PRINT_HILOGD("PrintServiceProxy, VendorId = %{public}d",
                   context->arrayPrinterExtensionInfo[i].GetVendorId());
      PRINT_HILOGD(
          "PrintServiceProxy, VendorName = %{public}s",
          context->arrayPrinterExtensionInfo[i].GetVendorName().c_str());
      PRINT_HILOGD("PrintServiceProxy, VendorIcon = %{public}d",
                   context->arrayPrinterExtensionInfo[i].GetVendorIcon());
      PRINT_HILOGD("PrintServiceProxy, Version = %{public}s",
                   context->arrayPrinterExtensionInfo[i].GetVersion().c_str());
      napi_value PrinterInfo;
      napi_create_object(env, &PrinterInfo);
      NapiPrintUtils::SetUint32Property(
          env, PrinterInfo, "extensionId",
          context->arrayPrinterExtensionInfo[i].GetExtensionId());
      NapiPrintUtils::SetUint32Property(
          env, PrinterInfo, "vendorId",
          context->arrayPrinterExtensionInfo[i].GetVendorId());
      NapiPrintUtils::SetStringPropertyUtf8(
          env, PrinterInfo, "vendorName",
          context->arrayPrinterExtensionInfo[i].GetVendorName().c_str());
      NapiPrintUtils::SetUint32Property(
          env, PrinterInfo, "vendorIcon",
          context->arrayPrinterExtensionInfo[i].GetVendorIcon());
      NapiPrintUtils::SetStringPropertyUtf8(
          env, PrinterInfo, "version",
          context->arrayPrinterExtensionInfo[i].GetVersion().c_str());
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    ...
    context->result = PrintManagerClient::GetInstance()->QueryAllExtension(
        context->arrayPrinterExtensionInfo);
  ...
  AsyncCall asyncCall(env, info,
                      std::dynamic_pointer_cast<AsyncCall::Context>(context));
...
napi_value NapiInnerPrint::StartDiscovery(napi_env env,
  ...
  auto input = [context](napi_env env, size_t argc, napi_value *argv,
    ...
    for (uint32_t i = 0; i < result_size; i++) {
      ...
      PRINT_HILOGD("output for :---- extensionVector value is :[%{public}d]",
                   valueNumber);
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    context->result = PrintManagerClient::GetInstance()->StartDiscoverPrinter(
        context->extensionVector);
    if (context->result == true) {
      context->status = napi_ok;
    }
  };
...
napi_value NapiInnerPrint::ConnectPrint(napi_env env, napi_callback_info info) {
  ...
  auto input = [context](napi_env env, size_t argc, napi_value *argv,
    ...
    NAPI_ASSERT_BASE(env, valuetype == napi_number,
                     "printerId number is not a number", napi_invalid_arg);
    uint32_t printId;
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    context->result =
        PrintManagerClient::GetInstance()->ConnectPrinter(context->printConnId);
    if (context->result == true) {
      context->status = napi_ok;
    }
  };
...
napi_value NapiInnerPrint::DisconnectPrint(napi_env env,
                                           napi_callback_info info) {
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    context->result = PrintManagerClient::GetInstance()->DisconnectPrinter(
        context->printDisConnId);
    if (context->result == true) {
      context->status = napi_ok;
    }
  };
...
napi_value NapiInnerPrint::StartPrintJob(napi_env env,
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    ...
    context->result = PrintManagerClient::GetInstance()->StartPrintJob(
        context->printStartJob);
    if (context->result == true) {
      context->status = napi_ok;
    }
  };
...
napi_value NapiInnerPrint::CancelPrintJob(napi_env env,
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    ...
    context->result = PrintManagerClient::GetInstance()->CancelPrintJob(
        context->printCancelJob);
    if (context->result == true) {
      context->status = napi_ok;
    }
  };
  context->SetAction(std::move(input), std::move(output));
  AsyncCall asyncCall(
      env, info, std::dynamic_pointer_cast<AsyncCall::Context>(context), 0);
  return asyncCall.Call(env, exec);
}
...
napi_value NapiInnerPrint::RequestPreview(napi_env env,
  ...
  auto exec = [context](AsyncCall::Context *ctx) {
    ...
    context->result = PrintManagerClient::GetInstance()->RequestPreview(
        context->printReqPreviewJob, context->previewResult);
    if (context->result == true) {
      context->status = napi_ok;
    }
  };
...
napi_value NapiInnerPrint::QueryCapability(napi_env env,
  ...
  auto input = [context](napi_env env, size_t argc, napi_value *argv,
    ...
    NAPI_CALL_BASE(
        env, napi_typeof(env, argv[NapiPrintUtils::INDEX_ZERO], &valuetype),
        napi_invalid_arg);
    NAPI_ASSERT_BASE(env, valuetype == napi_number, "number is not a number",
                     napi_invalid_arg);
    uint32_t number;
...
napi_value NapiInnerPrint::On(napi_env env, napi_callback_info info) {
  ...
  size_t len = 0;
  napi_get_value_string_utf8(env, argv[NapiPrintUtils::INDEX_ZERO], event,
                             NapiPrintUtils::MAX_LEN, &len);
  ...
  if (task->IsSupportPrinterStateType(type)) {
    PRINT_HILOGD("State type is Printer State. value is : %{public}s",
                 type.c_str());
  } else if (task->IsSupportJobStateType(type)) {
    PRINT_HILOGD("State type is Print Job State. value is : %{public}s",
                 type.c_str());
  } else {
...
napi_value NapiInnerPrint::Off(napi_env env, napi_callback_info info) {
  ...
  auto input = [context](napi_env env, size_t argc, napi_value *argv,
    ...
    NAPI_CALL_BASE(
        env, napi_typeof(env, argv[NapiPrintUtils::INDEX_ZERO], &valuetype),
        napi_invalid_arg);
    NAPI_ASSERT_BASE(env, valuetype == napi_string, "valuetype is not a number",
                     napi_invalid_arg);
    char buf[NapiPrintUtils::MAX_LEN] = {};
    size_t len = 0;
    napi_get_value_string_utf8(env, argv[NapiPrintUtils::INDEX_ZERO], buf,
                               NapiPrintUtils::MAX_LEN, &len);
...
sptr<IPrintCallback> NapiInnerPrint::CreateNotify(napi_env env,
