...
public:
  ...
  public:
    using InputAction =
        std::function<napi_status(napi_env, size_t, napi_value *, napi_value)>;
    ...
    virtual napi_status operator()(napi_env env, size_t argc, napi_value *argv,
                                   napi_value self) {
      if (input_ == nullptr) {
        return napi_ok;
      }
    ...
    void SetErrorIndex(uint32_t errorIndex) { errorIndex_ = errorIndex; }

    uint32_t GetErrorIndex() { return errorIndex_; }
  ...
  PrintAsyncCall(napi_env env, napi_callback_info info,
