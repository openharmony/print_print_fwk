...
{
    PRINT_HILOGI("Enter PrintEntry::StartPrint");
    std::vector<uint32_t> fdList;
    if (files.empty()) {
        PRINT_HILOGE("fileList and fdList are both empty");
        return E_PRINT_INVALID_PARAMETER;
    }
    auto fdListClearLambda = [&fdList, this]() {
        for (auto fd : fdList) {
            if (fd >= 0) {
                close(fd);
            }
    ...
    uint32_t ret = CallSpooler(adapterParam, files);
    if (ret != E_PRINT_NONE) {
        PRINT_HILOGE("CallSpooler error, ret = %d", static_cast<int32_t>(ret));
        fdListClearLambda();
        return ret;
    }
    ret = static_cast<uint32_t>(PrintManagerClient::GetInstance()->StartPrint(files, fdList, jobId));
    if (ret != E_PRINT_NONE) {
        PRINT_HILOGE("StartPrint error, ret = %d", static_cast<int32_t>(ret));
        fdListClearLambda();
        return ret;
    }
    fdListClearLambda();
