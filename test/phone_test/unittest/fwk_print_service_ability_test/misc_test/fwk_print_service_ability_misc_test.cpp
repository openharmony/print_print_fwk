...
HWTEST_F(PrintServiceAbilityTest, PrintServiceAbilityTestPartThree_ErrorToken_ShouldNoPermission, TestSize.Level1)
{
    ...
    auto permHelper = std::make_shared<MockPrintServiceHelper>();
    EXPECT_CALL(*permHelper, CheckPermission(_)).WillRepeatedly(Return(true));
    EXPECT_CALL(*permHelper, QueryAccounts(_))
        .WillRepeatedly(DoAll(SetArgReferee<0>(std::vector<int>{100}), Return(true)));
    service->SetHelper(permHelper);
...
HWTEST_F(PrintServiceAbilityTest, ParseSingleAdvanceOptJson_NonStringItem_Skipped, TestSize.Level1)
{
    auto service = PrintServiceAbilityTest::CreateService();
