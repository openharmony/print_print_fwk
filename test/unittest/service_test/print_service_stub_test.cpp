...
/**
 * @tc.name: PrintServiceStubTest_OnStartPrint_ReadStringTaskIdFails
 * @tc.desc: Verify OnStartPrint returns false when hasFdList is true with valid fds but taskId ReadString fails.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(PrintServiceStubTest, OnStartPrint_ReadStringTaskIdFails, TestSize.Level1)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
    ...
    // Intentionally NOT writing taskId string - ReadString will fail

    auto stub = std::make_shared<MockPrintService>();
    EXPECT_NE(stub, nullptr);
    EXPECT_FALSE(stub->OnStartPrint(data, reply));
}

/**
 * @tc.name: PrintServiceStubTest_OnAuthPrintJob_Success
 * @tc.desc: Verify OnAuthPrintJob returns true when AuthPrintJob returns E_PRINT_NONE.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(PrintServiceStubTest, OnAuthPrintJob_Success, TestSize.Level1)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
    ...
    EXPECT_TRUE(data.WriteInterfaceToken(IPrintService::GetDescriptor()));
    EXPECT_TRUE(data.WriteString("job123"));
    EXPECT_TRUE(data.WriteString("user1"));
    uint8_t passwordBuffer[MAX_AUTH_LENGTH_SIZE] = {0};
    passwordBuffer[0] = 't';
    passwordBuffer[1] = 'e';
    passwordBuffer[2] = 's';
    passwordBuffer[3] = 't';
    EXPECT_TRUE(data.WriteBuffer(passwordBuffer, MAX_AUTH_LENGTH_SIZE));

    auto stub = std::make_shared<MockPrintService>();
    EXPECT_NE(stub, nullptr);
    ON_CALL(*stub, AuthPrintJob(_, _, _)).WillByDefault(Return(E_PRINT_NONE));
    EXPECT_TRUE(stub->OnAuthPrintJob(data, reply));
}

/**
 * @tc.name: PrintServiceStubTest_OnAuthPrintJob_ReadBufferFails
 * @tc.desc: Verify OnAuthPrintJob returns false when ReadBuffer returns null.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(PrintServiceStubTest, OnAuthPrintJob_ReadBufferFails, TestSize.Level1)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option(MessageOption::TF_SYNC);
    ...
    EXPECT_TRUE(data.WriteString("job123"));
    EXPECT_TRUE(data.WriteString("user1"));
    // Don't write the password buffer - ReadBuffer will return nullptr

    auto stub = std::make_shared<MockPrintService>();
    EXPECT_NE(stub, nullptr);
    EXPECT_FALSE(stub->OnAuthPrintJob(data, reply));
}
}  // namespace Print
}  // namespace OHOS
