...
// ===== Audit-related (transient) field Tests =====

/**
 * @tc.name: PrintJobTest_SetFileList_001
 * @tc.desc: Verify SetFileList and GetFileList.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(PrintJobTest, PrintJobTest_SetFileList_001, TestSize.Level1)
{
    ...
    EXPECT_EQ(result[0], "/data/doc1.pdf");
    EXPECT_EQ(result[1], "/data/doc2.pdf");
}
...
HWTEST_F(PrintJobTest, PrintJobTest_SetFileList_002, TestSize.Level1)
{
    ...
    job.SetFileList(fileList);
    EXPECT_TRUE(job.GetFileList().empty());
}

/**
 * @tc.name: PrintJobTest_SetFileAuditInfo_001
 * @tc.desc: Verify SetFileAuditInfo and GetFileAuditInfo.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(PrintJobTest, PrintJobTest_SetFileAuditInfo_001, TestSize.Level1)
{
    ...
    EXPECT_EQ(result[0].size, 102400U);
    EXPECT_EQ(result[1].fileName, "doc2.pdf");
    EXPECT_EQ(result[1].md5, "098f6bcd4621d373cade4e832627b4f6");
    EXPECT_EQ(result[1].size, 204800U);
}
...
HWTEST_F(PrintJobTest, PrintJobTest_SetFileAuditInfo_002, TestSize.Level1)
{
    ...
    std::vector<FileAuditInfo> fileInfos;
    job.SetFileAuditInfo(fileInfos);
    EXPECT_TRUE(job.GetFileAuditInfo().empty());
}

/**
 * @tc.name: PrintJobTest_AuditCopyConstructor_001
 * @tc.desc: Verify copy constructor copies audit fields.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(PrintJobTest, PrintJobTest_AuditCopyConstructor_001, TestSize.Level1)
{
    PrintJob job;
    ...
    job.SetFileList(fileList);
    job.SetFileAuditInfo(fileInfos);
    job.SetJobId("job-audit-copy");

    PrintJob copyJob(job);
    EXPECT_EQ(copyJob.GetFileList().size(), 1U);
    EXPECT_EQ(copyJob.GetFileList()[0], "/data/doc1.pdf");
    EXPECT_EQ(copyJob.GetFileAuditInfo().size(), 1U);
    EXPECT_EQ(copyJob.GetFileAuditInfo()[0].fileName, "doc1.pdf");
    EXPECT_EQ(copyJob.GetFileAuditInfo()[0].md5, "d41d8cd98f00b204e9800998ecf8427e");
    EXPECT_EQ(copyJob.GetFileAuditInfo()[0].size, 102400U);
    EXPECT_EQ(copyJob.GetJobId(), "job-audit-copy");
}

/**
 * @tc.name: PrintJobTest_AuditAssignmentOperator_001
 * @tc.desc: Verify assignment operator copies audit fields.
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(PrintJobTest, PrintJobTest_AuditAssignmentOperator_001, TestSize.Level1)
{
    PrintJob job;
    std::vector<std::string> fileList = {"/data/doc1.pdf"};
    std::vector<FileAuditInfo> fileInfos = {
    ...
    job.SetFileList(fileList);
    job.SetFileAuditInfo(fileInfos);
    job.SetJobId("job-audit-assign");

    PrintJob assignJob = job;
    EXPECT_EQ(assignJob.GetFileList().size(), 1U);
    EXPECT_EQ(assignJob.GetFileList()[0], "/data/doc1.pdf");
    EXPECT_EQ(assignJob.GetFileAuditInfo().size(), 1U);
    EXPECT_EQ(assignJob.GetFileAuditInfo()[0].fileName, "doc1.pdf");
    EXPECT_EQ(assignJob.GetJobId(), "job-audit-assign");
}

/**
 * @tc.name: PrintJobTest_AuditTransient_001
 * @tc.desc: Verify audit fields are NOT serialized via Parcel (transient).
 * @tc.type: FUNC
 * @tc.require:
 */
HWTEST_F(PrintJobTest, PrintJobTest_AuditTransient_001, TestSize.Level1)
{
    PrintJob job;
    std::vector<std::string> fileList = {"/data/doc1.pdf"};
    std::vector<FileAuditInfo> fileInfos = {
    ...
    job.SetFileList(fileList);
    job.SetFileAuditInfo(fileInfos);
    job.SetJobId("job-transient-test");
    job.SetPrinterId("printer-001");
    ...
    auto unmarshalledJob = PrintJob::Unmarshalling(parcel);
    ASSERT_NE(unmarshalledJob, nullptr);
    // Audit fields should NOT be serialized
    EXPECT_TRUE(unmarshalledJob->GetFileList().empty());
    EXPECT_TRUE(unmarshalledJob->GetFileAuditInfo().empty());
    // Non-transient fields should be preserved
    EXPECT_EQ(unmarshalledJob->GetJobId(), "job-transient-test");
}
